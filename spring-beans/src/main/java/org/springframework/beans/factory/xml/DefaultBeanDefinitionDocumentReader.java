...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	 ...
	 */
	/**
	 * NOTE 2017-09-26 根据Spring DTD对Bean的定义规则解析Bean定义的Document对象
	 */
	@Override
	...
	/**
	 * NOTE 2017-09-26 创建BeanDefinitionParserDelegate 用于完成真正的解析过程
	 */
	protected BeanDefinitionParserDelegate createDelegate(XmlReaderContext readerContext,
			Element root, @Nullable BeanDefinitionParserDelegate parentDelegate) {
	...
	protected void parseBeanDefinitions(Element root,
		...
		if (delegate.isDefaultNamespace(root)) {
			...
			// Document的根节点没有使用Spirng默认的命名空间，则使用用户自定义的解析规则解析Document根节点
	...
	private void parseDefaultElement(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		// 元素既不是导入元素也不是别名元素，即普通的<Bean>元素,按照Spring的Bean规则解析元素
	 ...
	 */
	/**
	 * NOTE 2017-09-26 解析<Alias>别名元素，为Bean向Spring IoC容器注册别名
	 */
	protected void processAliasRegistration(Element ele) {
		// 获取<Alias>别名元素中name属性值
		String name = ele.getAttribute(NAME_ATTRIBUTE);
		// 获取<Alias>别名元素中alias属性值
